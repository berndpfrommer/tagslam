...
#include <stdexcept>
#include <tagslam/approx_image_sync.hpp>
  ...
  switch (inTopics.size()) {
    ...
    case 2:
      ...
      sync2_->registerCallback(
        boost::bind(&ApproxImageSync::callback2, this, _1, _2));
      break;
    case 3:
      sync3_.reset(
        new Sync3(ApproxPolicy3(sync_qs), *sub_[0], *sub_[1], *sub_[2]));
      sync3_->registerCallback(
        boost::bind(&ApproxImageSync::callback3, this, _1, _2, _3));
      break;
    case 4:
      sync4_.reset(new Sync4(
        ApproxPolicy4(sync_qs), *sub_[0], *sub_[1], *sub_[2], *sub_[3]));
      sync4_->registerCallback(
        boost::bind(&ApproxImageSync::callback4, this, _1, _2, _3, _4));
      break;
    case 5:
      sync5_.reset(new Sync5(
        ApproxPolicy5(sync_qs), *sub_[0], *sub_[1], *sub_[2], *sub_[3],
        *sub_[4]));
      sync5_->registerCallback(
        boost::bind(&ApproxImageSync::callback5, this, _1, _2, _3, _4, _5));
      break;
    case 6:
      sync6_.reset(new Sync6(
        ApproxPolicy6(sync_qs), *sub_[0], *sub_[1], *sub_[2], *sub_[3],
        *sub_[4], *sub_[5]));
      sync6_->registerCallback(
        boost::bind(&ApproxImageSync::callback6, this, _1, _2, _3, _4, _5, _6));
      break;
    case 7:
      sync7_.reset(new Sync7(
        ApproxPolicy7(sync_qs), *sub_[0], *sub_[1], *sub_[2], *sub_[3],
        *sub_[4], *sub_[5], *sub_[6]));
      sync7_->registerCallback(boost::bind(
        &ApproxImageSync::callback7, this, _1, _2, _3, _4, _5, _6, _7));
      break;
    case 8:
      sync8_.reset(new Sync8(
        ApproxPolicy8(sync_qs), *sub_[0], *sub_[1], *sub_[2], *sub_[3],
        *sub_[4], *sub_[5], *sub_[6], *sub_[7]));
      sync8_->registerCallback(boost::bind(
        &ApproxImageSync::callback8, this, _1, _2, _3, _4, _5, _6, _7, _8));
      break;
    default:
...
static sensor_msgs::ImageConstPtr modify_stamp(
  const ros::Time & stamp, const sensor_msgs::ImageConstPtr & imgMsg)
{
...
void ApproxImageSync::callback2(
  const ImageConstPtr & img0, const ImageConstPtr & img1)
{
...
void ApproxImageSync::callback3(
  const ImageConstPtr & img0, const ImageConstPtr & img1,
...
void ApproxImageSync::callback4(
  const ImageConstPtr & img0, const ImageConstPtr & img1,
  const ImageConstPtr & img2, const ImageConstPtr & img3)
{
  const ros::Time stamp = img0->header.stamp;
  pub_[0].publish(img0);
  pub_[1].publish(modify_stamp(stamp, img1));
...
void ApproxImageSync::callback5(
  const ImageConstPtr & img0, const ImageConstPtr & img1,
  const ImageConstPtr & img2, const ImageConstPtr & img3,
...
void ApproxImageSync::callback6(
  const ImageConstPtr & img0, const ImageConstPtr & img1,
  const ImageConstPtr & img2, const ImageConstPtr & img3,
  const ImageConstPtr & img4, const ImageConstPtr & img5)
{
  const ros::Time stamp = img0->header.stamp;
  pub_[0].publish(img0);
  pub_[1].publish(modify_stamp(stamp, img1));
...
void ApproxImageSync::callback7(
  const ImageConstPtr & img0, const ImageConstPtr & img1,
  const ImageConstPtr & img2, const ImageConstPtr & img3,
  const ImageConstPtr & img4, const ImageConstPtr & img5,
...
void ApproxImageSync::callback8(
  const ImageConstPtr & img0, const ImageConstPtr & img1,
  const ImageConstPtr & img2, const ImageConstPtr & img3,
  const ImageConstPtr & img4, const ImageConstPtr & img5,
  const ImageConstPtr & img6, const ImageConstPtr & img7)
{
  const ros::Time stamp = img0->header.stamp;
  pub_[0].publish(img0);
  pub_[1].publish(modify_stamp(stamp, img1));
...
void ApproxImageSync::callback9(
  const ImageConstPtr & img0, const ImageConstPtr & img1,
  const ImageConstPtr & img2, const ImageConstPtr & img3,
  const ImageConstPtr & img4, const ImageConstPtr & img5,
  const ImageConstPtr & img6, const ImageConstPtr & img7,
