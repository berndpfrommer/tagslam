...
TagSLAM::TagSLAM(const rclcpp::NodeOptions & options)
: Node("tagslam", rclcpp::NodeOptions(options))
{
}
...
static void poseEigenToMsg(const Eigen::Affine3d & e, geometry_msgs::Pose & m)
{
...
void TagSLAM::testForOldLaunchParameters()
{
  ...
  for (const auto & p : oldParams) {
    if (nh_.hasParam(p)) {
      BOMB_OUT(
        "error to specify " << p << " as launch param, must be in "
                            << "tagslam.yaml file now");
    }
  }
}
...
void TagSLAM::subscribe()
{
  ...
  if (hasCompressedImages_) {
    if (useApproximateSync_) {
      liveApproximateCompressedSync_.reset(new LiveApproximateCompressedSync(
        nh_, topics,
        std::bind(
    ...
    if (useApproximateSync_) {
      liveApproximateSync_.reset(new LiveApproximateSync(
        nh_, topics,
        std::bind(
...
void TagSLAM::doDump(bool optimize)
{
  ...
  publishTransforms(
    times_.empty() ? ros::Time(0)
                   : *(times_.rbegin()) + rclcpp::Duration(0.001),
    true);
...
void TagSLAM::readBodies(XmlRpc::XmlRpcValue config)
{
  ...
  for (const auto & body : bodies_) {
    ...
    if (!body->isStatic()) {
      ...
      trajectoryPub_.push_back(
        nh_.advertise<nav_msgs::Path>("path/body_" + body->getName(), QSZ));
...
void TagSLAM::readGlobalParameters(XmlRpc::XmlRpcValue config)
{
  ...
  useApproximateSync_ = xml::parse<bool>(
    config["tagslam_parameters"], "use_approximate_sync", false);
  syncQueueSize_ =
    xml::parse<int>(config["tagslam_parameters"], "sync_queue_size", 100);
  minTagArea_ =
    xml::parse<int>(config["tagslam_parameters"], "minimum_tag_area", 0);
  if (defbody.empty()) {
...
void TagSLAM::publishCameraTransforms(
  const ros::Time & t, tf::tfMessage * tfMsg)
{
  for (const auto & cam : cameras_) {
    Transform camTF;
...
void TagSLAM::publishTagAndBodyTransforms(
  const ros::Time & t, tf::tfMessage * tfMsg)
{
  geometry_msgs::TransformStamped tfm;
  for (const auto & body : bodies_) {
...
void TagSLAM::publishOriginalTagTransforms(
  const ros::Time & t, tf::tfMessage * tfMsg)
{
  geometry_msgs::TransformStamped tfm;
  for (const auto & body : bodies_) {
...
void TagSLAM::playFromBag(const string & fname)
{
  ...
  const ros::Time startTime =
    dummyView.getBeginTime() + rclcpp::Duration(deltaStartTime);
  ...
  if (hasCompressedImages_) {
    std::function<void(
      const std::vector<TagArray::ConstPtr> &,
      const std::vector<CompressedImage::ConstPtr> &,
      const std::vector<Odometry::ConstPtr> &)>
      cb = std::bind(
        &TagSLAM::syncCallbackCompressed, this, std::placeholders::_1,
        std::placeholders::_2, std::placeholders::_3);
    if (useApproximateSync_) {
      ...
      processBag<
        flex_sync::ExactSync<TagArray, CompressedImage, Odometry>, TagArray,
        CompressedImage, Odometry>(&view, topics, cb);
    }
  } else {
    std::function<void(
      const std::vector<TagArray::ConstPtr> &,
    ...
    if (useApproximateSync_) {
      processBag<
        flex_sync::ApproximateSync<TagArray, Image, Odometry>, TagArray, Image,
        Odometry>(&view, topics, cb);
    } else {
      processBag<
        flex_sync::ExactSync<TagArray, Image, Odometry>, TagArray, Image,
        Odometry>(&view, topics, cb);
    }
  }
  bag.close();
  ROS_INFO_STREAM(
    "done processing bag, total wall time: "
...
static nav_msgs::Odometry make_odom(
  const ros::Time & t, const std::string & fixed_frame,
...
void TagSLAM::fakeOdom(
  const ros::Time & tCurr, std::vector<VertexDesc> * factors)
{
  if (!times_.empty()) {
...
void TagSLAM::processOdom(
  const ros::Time & t, const std::vector<OdometryConstPtr> & odomMsgs,
  std::vector<VertexDesc> * factors)
{
...
void TagSLAM::processTags(
  const ros::Time & t, const std::vector<TagArrayConstPtr> & tagMsgs,
  std::vector<VertexDesc> * factors)
{
  ...
  for (const auto i : irange(0ul, cameras_.size())) {
    ...
    if (!tags.empty()) {
      ...
      RelativePosePriorFactorPtr fac(
        new factor::RelativePosePrior(t, ros::Time(0), pn, name));
...
void TagSLAM::remapAndSquash(
  const ros::Time & t, std::vector<TagArrayConstPtr> * remapped,
...
void TagSLAM::readRemap(XmlRpc::XmlRpcValue config)
{
  ...
  if (remap.getType() == XmlRpc::XmlRpcValue::TypeArray) {
    ...
    for (const auto i : irange(0, remap.size())) {
      if (remap[i].getType() != XmlRpc::XmlRpcValue::TypeStruct) continue;
      int remapId = -1;
        ...
        if (it->first == "remaps") {
          ...
          if (re.getType() == XmlRpc::XmlRpcValue::TypeArray) {
            for (const auto j : irange(0, re.size())) {
              auto a = re[j];
              if (a.getType() != XmlRpc::XmlRpcValue::TypeStruct) continue;
              ReMap r(
                static_cast<int>(a["remap_id"]),
...
static void insert_into(
  std::map<ros::Time, std::set<int>> * map, const ros::Time & t,
...
void TagSLAM::parseTimeSquash(
  XmlRpc::XmlRpcValue sq, const ros::Time & t, const std::set<int> & tags)
{
...
void TagSLAM::writeTagCorners(
  const ros::Time & t, int camIdx, const TagConstPtr & tag,
